 ...
 * Modify:1、2014-08-06添加促销模块方法GetPlanMaster
 * ******************************************************/
namespace LL.POS
{
    using System;
    ...
    public class PosDal : DalBase
    {
        ...
        /// 获取商品大类列表
        /// </summary>
        /// <param name="clsNo">类ID</param>
        /// <param name="pageSize">页数</param>
        /// <param name="isNextPages">是否分页</param>
        /// <returns></returns>
        ...
        /// <param name="pendingOrderInfo"></param>
        /// <returns></returns>
        ...
        /// 临时销售行构建命令
        /// </summary>
        /// <param name="saleflow">流水表数据</param>
        /// <returns></returns>
        ...
        /// <returns>如果CardNo=""返回所有数据,否则模糊查询</returns>
        ...
        /// 退货原因信息获取
        /// </summary>
        /// <param name="flag">原因是否在用,默认为空</param>
        /// <returns></returns>
    ...
    " ( SELECT a.item_no,a.flow_no,a.unit_price,a.sale_price,a.sale_way,a.discount_rate,a.item_subno,a.item_clsno,a.item_name,a.item_status,a.item_subname,a.sale_qnty - ifnull(b.sale_qnty,0) AS sale_qnty,a.unit_price * (a.sale_qnty - b.sale_qnty) AS sale_money,a.price,item_brand  FROM " +
        ...
        /// 获取最新的支付信息
        /// </summary>
        /// <param name="_branchNo"></param>
        /// <param name="_posNo"></param>
        /// <returns></returns>
        ...
        /// 更新增量更新的标识信息
        /// </summary>
        /// <param name="_t_handle">增量更新标识实体</param>
        /// <returns>更新结果，true为成功</returns>
        ...
        /// 根据商品信息更新本地库存和服务器库存
        /// </summary>
        /// <param name="item_no">商品编码</param>
        /// <param name="order_qty">商品信息</param>
        ...
        public bool UpdateItemStockByItem(string item_no, decimal order_qty, string db_no)
        {
            ...
            try
            {
                ...
                if (errorMessage.Length > 0)
                {
                    LoggerHelper.Log("MsmkLogger", "LL.POS->PosDal-->UpdateItemStockByItem--->>>>Error:" + errorMessage, LogEnum.ExceptionLog);
                }
            }
            catch (SQLiteException sqlex)
            {
                LoggerHelper.Log("MsmkLogger", "LL.POS->PosDal-->UpdateItemStockByItem--->>>>" + sqlex.ToString(), LogEnum.ExceptionLog);
            }
            catch (Exception ex)
            {
                LoggerHelper.Log("MsmkLogger", "LL.POS->PosDal-->UpdateItemStockByItem--->>>>" + ex.ToString(), LogEnum.ExceptionLog);
            }
            return isok;
        }
        /// <summary>
        /// 插入促销计划规则
        /// </summary>
        /// <param name="table">促销计划规则信息</param>
        /// <returns>成功TRUE失败FALSE</returns>
...
"	) b             ";
        ...
        /// 根据促销规则计划获取促销的详细信息
        /// </summary>
        /// <param name="table">促销规则主信息</param>
        /// <returns></returns>
     ...
     " LEFT JOIN t_product_food_barcode b ON f.item_no = b.item_no WHERE b.item_barcode = '{0}'";
        ...
        public bool AddVipflow(t_app_viplist vipinfo)
        {
            ...
            try
            {
                ...
                if (errorMessage.Length > 0)
                {
                    LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->AddVipflow--->Error:" + errorMessage, LogEnum.ExceptionLog);
                }
                if (isok == false)
                {
                    ...
                    LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->AddVipflow--->Data---->Detail:" + _sb.ToString(), LogEnum.ExceptionLog);
                }
            }
            catch (SQLiteException sqlex)
            {
                LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->AddVipflow--->SQLiteException:" + sqlex.ToString(), LogEnum.ExceptionLog);
            }
            catch (Exception ex)
            {
                LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->AddVipflow--->Exception:" + ex.ToString(), LogEnum.ExceptionLog);
            }
            return isok;
        }
        /// <summary>
        ...
        public bool DeleteTempNonSaleData()
        {
            ...
            try
            {
                ...
                if (errorMessage.Length > 0)
                {
                    LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->DeleteTempNonSaleData--->Error:" + errorMessage, LogEnum.ExceptionLog);
                }
            }
            catch (SQLiteException sqlex)
            {
                LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->DeleteTempNonSaleData--->SQLiteException:" + sqlex.ToString(), LogEnum.ExceptionLog);
            }
            catch (Exception ex)
            {
                LoggerHelper.Log(Gattr.LoggerName, "LL.POS->PosDal-->DeleteTempNonSaleData--->SQLiteException:" + ex.ToString(), LogEnum.ExceptionLog);
            }
            return isok;
        }
